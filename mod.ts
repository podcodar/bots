...
import {
	addDailyRecord,
	computeDailyScoreboardPoint,
} from '@usecases/dailyBot.ts';
...
const baseBot = createBot({
	...
	events: {
		...
		async messageCreate(bot, message) {
			const isDailyChannel =
				message.channelId === BigInt(settings.DAILY_CHANNEL);
			if (!isDailyChannel) return;

			const content = message.content.toLocaleLowerCase();
			const isDailyMessage = content.includes('o que eu fiz') &&
			...
			const { error, data: dailyScoreboard } =
				await computeDailyScoreboardPoint(userId, name);
			if (error != null) {
				console.error(error);
				return;
			}
