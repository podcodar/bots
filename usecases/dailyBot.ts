...
import { CreateDailyRecord, DailyRecord, CreateDailyScoreboard, DailyScoreboard } from "@entities";
...
export async function computeDailyScoreboardPoint(userId: string, name: string) {
...
function upsertDailyScoreboard(dailyScoreboard: CreateDailyScoreboard | DailyScoreboard) {
  return client.from<DailyScoreboard>("daily_scoreboard").upsert([dailyScoreboard]);
}

function getScoreboardByUserId(userId: string) {
  return client.from<DailyScoreboard>("daily_scoreboard").select("*").eq("userId", userId);
}
